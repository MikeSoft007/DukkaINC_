...
class Generate(Resource):
    ...
    def post(self):
        ...
        pushData = DumpData()
        pushData_ = pushData.send_data(data['name'], data['address'], data['phone_number'],
                                    data['price'], data['item'], data['qty'], data['price']*data['qty'])

        if pushData_:
            return jsonify({"Message": "Receipt successfully generated", "status":200})
        else:
            return jsonify({"Message": "Not generated, kindly check input", "status": 404})
